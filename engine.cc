...
std::vector<Square> Engine::bestMove(Moves mve, int depth)
{
    char boardCpy[8][8];
    Square bestEndPos = get_pos(-1, -1);
    Square bestStartPos = get_pos(-1, -1);
    int alpha = -MAX_INT;
    int beta = MAX_INT;
    ...
    for(Square pos: get_moves.getPositions(true))
    {
        for(Square move: get_moves.filterMoves(pos, get_moves.get_moves(pos.y, pos.x)))
        {
    ...
    std::vector<Square> bestMoves {bestStartPos, bestEndPos};

    return bestMoves;
}

int Engine::miniMax(Moves mve, bool maximizing, int depth, int alpha, int beta)
{
    char boardCpy[8][8];
    for(int i=0;i<8;i++)
    {
    ...
    bool prune = false;
    for(Square pos: testMoves.getPositions(maximizing))
    {
        for(Square move: testMoves.filterMoves(pos, testMoves.get_moves(pos.y, pos.x)))
        {
...
std::vector<Square> Engine::getPositions(char boardCpy[8][8], bool maximizing)
{
...
int Engine::evaluation(char boardCpy[8][8])
{
    int blackScore = 0;
    int whiteScore = 0;
    Moves mve(boardCpy);
